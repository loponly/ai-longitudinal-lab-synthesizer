"""Main synthesizer that orchestrates the lab data processing pipeline using Langroid agents."""
import json
import os
...
from langroid.language_models import OpenAIGPTConfig, MockLMConfig
...
class LabDataSynthesizer:
    """Main orchestrator for lab data synthesis and report generation using Langroid agents."""
    ...
    def process_patient_data(self, patient_data: PatientData) -> PatientSummary:
        ...
        # Step 2: Use Langroid agent for intelligent classification
        lab_data_str = self._format_lab_data_for_agent(patient_data.labs)
        
        classification_task = Task(
            self.classifier_agent,
            name="classify_labs",
            system_message="Classify the following lab results into health domains and assess abnormalities:",
        )
        
        classification_result = classification_task.run(lab_data_str)
        
        # Step 3: Use traditional grouping but enhance with agent insights
        ...
        # Step 4: Use Langroid agent for trend analysis
        health_summaries = []
        for domain, labs in domain_groups.items():
            if domain != HealthDomain.OTHER or labs:
                # Use agent for trend analysis
                trend_data = self._format_domain_data_for_trend_analysis(domain, labs)
                
                trend_task = Task(
                    self.trend_agent,
                    name=f"analyze_trends_{domain.value}",
                    system_message=f"Analyze trends for {domain.value} domain lab results:",
                )
                
                trend_result = trend_task.run(trend_data)
                
                # Create health summary with agent-enhanced analysis
                health_summary = self._create_enhanced_health_summary(domain, labs, trend_result.content)
                health_summaries.append(health_summary)

        # Step 5: Use Langroid agent for overall summary generation
        overall_summary = self._generate_overall_summary_with_agent(health_summaries, patient_data)
    ...
    def _create_enhanced_health_summary(self, domain: HealthDomain, labs, agent_analysis: str) -> HealthSummary:
    ...
    def _generate_overall_summary_with_agent(self, health_summaries: List[HealthSummary], patient_data: PatientData) -> str:
        ...
        summary_task = Task(
            ...
            system_message="Generate a concise overall clinical summary with key findings and recommendations:",
        )
    ...
    def synthesize_to_formats(self, json_data: str) -> Dict[str, Any]:
        """Process input and return reports in multiple formats using Langroid agents."""
        # Parse JSON input
        data = json.loads(json_data) if isinstance(json_data, str) else json_data
